...
-   `run_par`
    ...
    Holding all game parameters for the GMSE simulation for a session. As per section **XXX**, in the example application presented here, the majority of these will be constants, with only `ownership_var` and `remove_pr` varying per session.
...
We chose to focus on a scenario that varies one focal parameter (farmer land ownership distribution).
